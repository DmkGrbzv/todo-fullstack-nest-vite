...
import { ITask } from "../interfaces/todo";
  ...
  methods:{
    async getCheckBoxValue(value:boolean){
      ...
      await this.fetchUpdateTask({ text:this.itemData.text, completed: this.isSelected, _id: this.itemData._id }, this.itemData._id )
    },
  ...
  <div class="item-wrapper">
    <div class="item">
      <label for="" class="item__label" :class="{'item__label-done':  isSelectedValue}">
        ...
        <input ref="item" type="text" v-model="value" v-show="!isEditMode" @change.stop="getTextValue(value)" :class="{'item__label-done': isSelectedValue}" v-on:focusout="isEditMode = true">
