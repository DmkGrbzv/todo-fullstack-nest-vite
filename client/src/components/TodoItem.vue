...
import { ITask } from "../interfaces/todo";
  ...
  methods:{
    async getCheckBoxValue(value:boolean){
      ...
      await this.fetchUpdateTask({ text:this.value, completed: this.isSelected, _id: this.itemData._id }, this.itemData._id )
    },
    async getTextValue(value:string){
      await this.fetchUpdateTask({ text:value,completed:this.isSelected,_id:this.itemData._id }, this.itemData._id);
    },
    focusEdit(){
  ...
  computed:{
    ...
    isChangedTextValue(){
      if(this.itemData.text && !(this.value.length>0)){
  ...
  <div class="item-wrapper">
    <div class="item">
      <label for="" class="item__label" :class="{'item__label-done':  isSelectedValue}">
        ...
        <input ref="item" type="text" v-model="value" v-show="!isEditMode" @change="getTextValue(value)" :class="{'item__label-done': isSelectedValue}" v-on:focusout="isEditMode = true">
        <p @dblclick="editField(isEditMode)" v-show="isEditMode">{{isChangedTextValue}}</p>
